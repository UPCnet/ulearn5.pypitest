...
class IPypitestUserSchema(model.Schema):
    dni = schema.TextLine(
...
class PypitestUserDataSchemaAdapter(AccountPanelSchemaAdapter):
    schema = IPypitestUserSchema
...
class PypitestUserDataPanelExtender(extensible.FormExtender):
    adapts(Interface, IUlearn5PypitestLayer, UserDataPanel)

    def update(self):
        fields = field.Fields(IPypitestUserSchema)
...
class PypitestRegistrationPanelExtender(extensible.FormExtender):
    adapts(Interface, IUlearn5PypitestLayer, BaseRegistrationForm)
